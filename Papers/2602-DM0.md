...
## Connections
- 直接对比: [[2410-Pi0|pi0]]（同为 VLM + flow matching action expert 架构，但 DM0 从预训练阶段就融入 embodied data，而 pi0 遵循 pretrain-then-adapt）、[[2504-Pi05|pi0.5]]（在 Table30 上被 DM0 超越，42.67% vs. 62%）
- VLA 架构对比: [[2406-OpenVLA]]（autoregressive action 预测的代表，DM0 采用 hybrid discrete/continuous 方案）、[[2502-OpenVLA-OFT]]（研究 VLA fine-tuning 优化，DM0 的 gradient decoupling 是另一种防止能力退化的方案）、[[2506-SmolVLA]]（同样追求小参数量高效率，450M vs. DM0 2B）
- Navigation 相关: [[2402-NaVid]]（video-based VLN）、[[2412-NaVILA]]（navigation VLA with mid-level language actions）、[[2412-LHVLN]]——DM0 试图在单一模型内统一 navigation 和 manipulation，而这些工作专注 navigation
- Mobile manipulation: [[2401-MobileALOHA]]（bimanual mobile manipulation 数据收集和 co-training，DM0 的 mid-training 也使用了 ALOHA 数据）
- 学习范式: [[2511-PiStar06|pi*0.6]]（通过 RL 从部署经验中自我改进，DM0 目前仅用 imitation learning，RL 是潜在的未来方向）
