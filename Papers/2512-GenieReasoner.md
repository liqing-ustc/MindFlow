...
GenieReasoner 通过 FACT tokenizer 将连续动作离散化并借助 flow matching 重建高保真轨迹，在统一自回归 VLA 框架中同时解决了 embodied reasoning 和 action precision 的 trade-off。
...
- **模型权重**: 未发布
- **训练细节**: 高层描述（三阶段训练、数据来源列表），缺少具体超参、数据配比、训练步数
