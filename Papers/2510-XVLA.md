...
## Connections
- [[2410-Pi0]]：π₀ 同样采用 flow matching + cross-embodiment 训练范式，但使用独立 action expert（300M）处理动作，X-VLA 则通过 soft prompt 在统一 backbone 中处理异构性，架构更简洁；cloth folding 性能与 π₀ 可比
- [[2405-Octo]]：Octo 是早期开源 cross-embodiment generalist policy，采用 diffusion action head，X-VLA 在多个 benchmark 上超越 Octo
- [[2406-OpenVLA]]：OpenVLA 使用 autoregressive token 预测动作，X-VLA 采用 flow matching 建模连续分布，在灵巧操作上优势明显
- [[2506-SmolVLA]]：SmolVLA 同样追求参数效率和 flow matching，但专注于单一平台，X-VLA 重点解决 cross-embodiment 问题
- [[2504-Pi05]]：π₀.5 是 π₀ 的后续工作，进一步 scale up cross-embodiment 训练
- [[2412-RoboVLMs]]：RoboVLMs 系统综述了 VLA 架构设计，X-VLA 的 soft prompt 策略为其中 heterogeneity handling 提供了新方案
- [[2502-OpenVLA-OFT]]：OpenVLA-OFT 探索了 VLA 的高效 fine-tuning，与 X-VLA 的 1% 参数 fine-tuning 策略形成对比
